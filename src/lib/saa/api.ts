...
export interface SubmissionPayload {
  ...
  marketing_list_id: string; // Comma separated IDs
  marketing_supplier_id: string; // Comma separated Supplier IDs
...
export function buildPayload(
  ...
  // Use passed available items or default to hardcoded if not provided (though dynamic is preferred)
  const itemsSource = availableMarketingItems || [];

  const selectedMarketingItems = formData.selectedMarketing
    .map((id) => itemsSource.find((m) => m.id === id))
  ...
  const payload: SubmissionPayload = {
    ...
    marketing_total_cost: `$${formatNumberWithCommas(
      String(calculateMarketingTotal(formData.selectedMarketing, itemsSource)),
    )}`,
    marketing_total_cost_in_words: dollarAmountToWords(
      calculateMarketingTotal(formData.selectedMarketing, itemsSource),
      false,
    ),
...
export async function submitForm(
  ...
  const payload = buildPayload(formData, availableMarketingItems);
